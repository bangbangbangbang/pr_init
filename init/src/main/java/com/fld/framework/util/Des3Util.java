...
public class Des3Util extends SecurityUtil {
	 ...
	 * @return
	 */
	public static String decryptWithURLDecoder(String result) {
		if (null != result && !result.isEmpty()) {
			try {
				result = URLDecoder.decode(result, "UTF-8");
			} catch (UnsupportedEncodingException e) {
			...
			int i = Integer.parseInt(result.substring(1, 2));
			String key = result.substring(i + 1, i + 1 + 24);
			result = result.substring(0, 1) + result.substring(2, i + 1) + result.substring(i + 1 + 24);
			return decrypt(result, key);
		}
		return result;
	}
	 ...
	 * @return
	 */
	public static String encryptWithURLEncoder(String body) {
		String key = GenerateUtil.generateSecretKey();
		String result = encrypt(body, key);
		int i = new Random().nextInt(8) + 2;
		result = result.substring(0, 1) + i + result.substring(1, i) + key + result.substring(i);
		if (null != result && !result.isEmpty()) {
			try {
				return URLEncoder.encode(result, "UTF-8");
			} catch (UnsupportedEncodingException e) {
