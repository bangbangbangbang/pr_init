...
public class IpUtils {
	...
	public static boolean checkAuthIP(Long[][] authIps, String ip) {
		...
		if (authIps == null) {
			...
			String[] ipb = ip.split("\\.");
			Long iip = (Long.valueOf(ipb[0]) << 24) + (Long.valueOf(ipb[1]) << 16) + (Long.valueOf(ipb[2]) << 8) + Long.valueOf(ipb[3]);
		...
		for (int i = 0; i < iparray.length; i++) {
			if (iparray[i] != null && !"".equals(iparray[i])) {
				...
				if (ipzone.length == 2) {
					...
					iip = (Long.valueOf(ipb[0]) << 24) + (Long.valueOf(ipb[1]) << 16) + (Long.valueOf(ipb[2]) << 8) + Long.valueOf(ipb[3]);
					result[i][0] = iip;
					iip = (Long.valueOf(ipe[0]) << 24) + (Long.valueOf(ipe[1]) << 16) + (Long.valueOf(ipe[2]) << 8) + Long.valueOf(ipe[3]);
					result[i][1] = iip;
				} else {
	...
	public static Long parseIp(String ip) {
		...
		result = (Long.valueOf(iparray[0]) << 24) + (Long.valueOf(iparray[1]) << 16) + (Long.valueOf(iparray[2]) << 8) + Long.valueOf(iparray[3]);
		return result;
	}
