...
public class DateUtils implements Serializable {
	 ...
	 * yyyyMMddHHmmss
	 */
	public static final String TIMESTAMP_FORMAT_YYYYMMDDHHMMSS = "yyyyMMddHHmmss";
	 ...
	 * @return
	 */
	public static String formatDateYYYYMMDD(Integer secend) {
	...
	/****
	 * 标准日期格式 转换为 uinx时间戳 标准日期格式格式为 yyyyMMddHHmmss
	 * 
	 * @return 单位：秒
	 */
	public static Integer getUnixTimeYyyyMMddHHmmss(String fmtDate) {
		if (fmtDate == null || fmtDate.length() == 0) {
			return null;
		}
		SimpleDateFormat sdf = new SimpleDateFormat(TIMESTAMP_FORMAT_YYYYMMDDHHMMSS);
		try {
			return (int) (sdf.parse(fmtDate).getTime() / 1000);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static boolean isSameWeak(Long firstDate, Long secondDate) {
		...
		Week w = getWeek(new Date(firstDate));
		// 比较第二个时间是否与第一个时间在同一周
	 ...
	 * @return
	 */
	public static String whatWeek(Integer second) {
		Calendar c = Calendar.getInstance();
		c.setTimeInMillis(1000L * second.longValue());
	 ...
	 * @return
	 */
	public static int whatWeekDay(Integer second) {
		Calendar c = Calendar.getInstance();
		c.setTimeInMillis(1000L * second.longValue());
