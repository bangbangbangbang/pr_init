...
public abstract class SecurityUtil {
	...
	public String encryptToBase64(String key, String src, String encoding) {
		try {
			return new String(org.apache.commons.codec.binary.Base64.encodeBase64(encrypt(key.getBytes(), src.getBytes(encoding))));
		} catch (Exception ex) {
			return null;
		}
	}
	...
	public String decryptFromBase64(String key, String src, String encoding) {
		try {
			return new String(decrypt(key.getBytes(), org.apache.commons.codec.binary.Base64.decodeBase64(src.getBytes())), encoding);
		} catch (Exception ex) {
			return null;
			// ex.printStackTrace();
		}
	...
	public String encryptToBase64DefaultEncoding(String key, String src) {
		try {
			return new String(org.apache.commons.codec.binary.Base64.encodeBase64URLSafe(encrypt(key.getBytes(), src.getBytes())));
		} catch (Exception ex) {
			ex.printStackTrace();
		}
		return null;
	...
	public String decryptFromBase64DefaultEncoding(String key, String src) {
		try {
			return new String(decrypt(key.getBytes(), org.apache.commons.codec.binary.Base64.decodeBase64(src.getBytes())));
		} catch (Exception ex) {
			ex.printStackTrace();
			return null;
		}
