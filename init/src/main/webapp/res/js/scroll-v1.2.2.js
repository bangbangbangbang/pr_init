	...
	$.fn.extend({
		scrollEve:function(options){
			...
			$(ops.selectors.sure).on("click",function(){
				...
				var m = $(ops.selectors.scrollitem).eq(1).find(ops.selectors.active).text();
				var d = $(ops.selectors.scrollitem).eq(2).find(ops.selectors.active).text();
				if(m<=9){
					m = "0"+m;
				}
				if(d<=9){
				...
				var str = $(ops.selectors.scrollitem).eq(0).find(ops.selectors.active).text()+"年"+m+"月"+d+"日"+$(ops.selectors.scrollitem).eq(3).find(ops.selectors.active).text();
				var i = parseInt($("body").attr("data-ix"));
				$(obj).children().eq(i).find(ops.selectors.obj).text(str);
				$(obj).addClass("active");
				$(".scroll").fadeOut();
				$(obj).addClass("active");
			})
