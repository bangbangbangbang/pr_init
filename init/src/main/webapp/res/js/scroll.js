	...
	$.fn.extend({
		scrollEve:function(options){
			...
			$(ops.selectors.sure).on("click",function(){
				...
				var str = $(ops.selectors.scrollitem).eq(0).find(ops.selectors.active).text()+"年"+$(ops.selectors.scrollitem).eq(1).find(ops.selectors.active).text()+"月"+$(ops.selectors.scrollitem).eq(2).find(ops.selectors.active).text()+"日\t\/\t"+$(ops.selectors.scrollitem).eq(3).find(ops.selectors.active).text();
				$(obj).text(str);
				$(obj).addClass("active");
				$(".scroll").fadeOut();
				$(obj).addClass("active");
