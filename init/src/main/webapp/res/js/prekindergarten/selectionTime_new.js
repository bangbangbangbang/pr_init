$(function(){
   ...
   $(".loadTable p.timeLeft").click(function(){
        if(!$('div#tableDetails').hasClass('tableEdit')) {
                return;
      }
          // 获取每项任务的自定义id
                    var index = $(this).parent('div').parent('div').parent('div')[0].dataset.id;
                    // 获取到每项任务名称的值
                    var titie = $(this).parent('div').parent('div').parent('div').children('.timeHeaderImg').find('p').eq(0).html();
                    // 获取图片
                    ...
                    //将时间右边的标题设置为页面中的标题
                    $('.headerLeft span').eq(0).html(titie);
                    ...
                    $('.choiceDate').css('position','absolute');
                    $('.choiceDate').css('top',topDistance); 
                    $(window).scroll(function(){
                    	if($(window).scrollTop()>=topDistance){
                    	$('.choiceDate').css('top',topDistance);
                    	}else{
                    	$('.choiceDate').css('top',0); 
                    	}
                    	})
                    objLeft=$(this);
                    // 左边时间的值
		...
		$(".loadTable p.timeRight").click(function(){
            if(!$('div#tableDetails').hasClass('tableEdit')) {
                return;
            }
			 // 获取每项任务的自定义id
