...
<head>
	...
	<style>
		.active .conL,.active .conR{
			color: #ff9900 !important;
		}
		.wrap .public .testDivision .testMess .messRight .rightContent .conL {
			    line-height: 0.6rem;
			}
		.wrap .public .testDivision .testMess .messRight .rightContent .conR {
		    font-size: 0.38rem;
		}
	</style>
</head>
        ...
        <div class="public">
            <div class="testDivision">
                <div class="testMess clearfix">
                    <!--左边测试人的信息-->
                    <div class="messLeft">
                    	<c:if test="${not empty babyGoSchoolTime}">
                    		<span class="testTime">入园时间：${babyGoSchoolTime}</span>
                    	</c:if>
                    	<c:if test="${not empty babynickname}">
                    ...
                    <div class="choiceCycle clearfix commit" style="display:none">
                        ...
                        <div class="costMon testSub comment" >
                            <span class="costNum ">提&nbsp;&nbsp;交</span>
                        </div>
                    </div>
                    <div style="display:none" class="testTimeBtn">总用时  3分 23秒</div>
                </div> 
            </div>
        </div>     
    </div>
			...
			if(!(id&&dimensionId&&problemId)){
			 ...
			 HH.ajax({
				    ...
				    callback:function(data){
				   		if(data.success){
				   			HH.href("/yzmm/account/activity/prekindergarten/toEnlistPagePay?cpResultId="+cpResultId+"&forcpindex=false");
			   			}else{
			   				toast(data.msg);
			   			}
				    }
		...
		**/
		$('.rightContent').on('click',function(){
			...
			if(index==messRightLen-1){
				...
				$('.testTimeBtn').html('总用时 '+formatSeconds(time/1000)).show();
			}
		});
		...
		//toast("此测评每人仅一次机会，且结果与入园适应训练方案相关，请根据宝宝实际情况进行测评。",null,5000);
	});
		   			...
		   			//是否为空
						...
						for (var i = 0; i < len; i++) {
							...
							for(var j = 0;j<optLen;j++){
								...
								$('<p class="conL">'+opt.optionCode+'</p>').appendTo(rightContent);
								$('<p  class="conR"  data-id="'+opt.id+'" data-subjectid="'+opt.subjectId+'" data-dimensionid="'+opt.dimensionId+'" data-problemid="'+opt.problemId+'" data-score="'+opt.score+'">'+opt.optionContent+'</p>').appendTo(rightContent);
							}
						}
		   			}
			    }
